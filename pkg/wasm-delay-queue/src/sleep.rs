...
pub fn sleep(duration: Duration) -> Sleep {
    match Instant::now().checked_add(duration) {
        Some(deadline) => Sleep::new_timeout(deadline, None),
        None => Sleep::new_timeout(Instant::far_future(), None),
    }
}
...
pin_project! {
    ...
    pub struct Sleep {
        ...
        #[pin]
        entry: Pin<Box<dyn Future<Output=()> + Send>>,
    }
}
...
impl Sleep {
    ...
    pub fn reset(mut self: Pin<&mut Self>, deadline: Instant) {
        *self = Sleep::new_timeout(deadline, None);
    }
}
...
impl Future for Sleep {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
        let result = self.entry.as_mut().poll(cx);
        result
    }
}
...
impl fmt::Debug for Sleep {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Delay with deadline {:?}", self.inner.deadline)
    }
}
