...
docker build -f wasm.dockerfile -t "github.com/amurant/wasm_controller:${local_tag}" .

kind load docker-image --name "${KIND_CLUSTER_NAME}" "github.com/amurant/wasm_controller:${local_tag}"

helm upgrade master-tim "${SCRIPT_ROOT}/../deploy/chart/" \
	--install \
	--namespace controller-namespace \
	--create-namespace \
	--set image="github.com/amurant/wasm_controller:${local_tag}"
