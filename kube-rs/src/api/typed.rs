...
use crate::{
    api::{
        DeleteParams, ListParams, Meta, ObjectList, PatchParams, PostParams, Resource, WatchEvent,
    },
    ...
    K: Clone + DeserializeOwned + Meta,
{
    ...
    /// }
    /// ```
    pub fn watch(&self, lp: &ListParams, version: &str) -> Result<Vec<Result<WatchEvent<K>>>> {
        let req = self.resource.watch(&lp, &version)?;
        self.client.request_events::<K>(req)
    }
}
