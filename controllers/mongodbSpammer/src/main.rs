...
async fn main() {
    ...
    for _i in 0..100000 {
        ...
        println!("{:?} reads per sec", totalReads as f64 / elapsed.as_secs_f64() );
    }
...
async fn get_connection() -> Result<Database,mongodb::error::Error>{
    ...
    let client = Client::with_uri_str(&env::var("MONGO_URI").unwrap_or("mongodb://root:password@localhost:27017".to_string())).await?;
